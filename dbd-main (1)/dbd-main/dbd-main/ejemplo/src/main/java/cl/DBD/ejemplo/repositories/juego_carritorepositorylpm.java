...
public class juego_carritorepositorylpm implements  juego_carritorepository{
    ...
    public void actualizarJuegoEnCarrito(int id_carrito, int id_juego, int nuevoIdJuego) {
        try (Connection conn = sql2o.open()) {
            String sql = "UPDATE juegos_carrito SET id_juego = :nuevoIdJuego WHERE id_carrito = :id_carrito AND id_juego = :id_juego";
            conn.createQuery(sql)
                    .addParameter("id_carrito", id_carrito)
                    .addParameter("id_juego", id_juego)
                    .addParameter("nuevoIdJuego", nuevoIdJuego)
                    .executeUpdate();
        } catch (Exception e) {
            System.out.println("Error al actualizar juego en carrito: " + e.getMessage());
        }
    }
}
