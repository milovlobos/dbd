...
public class juego_restriccionesrepositorylpm implements juego_restriccionesrepository{
    ...
    public void actualizarRestriccionEnVideojuego(int id_videojuego, int id_restriccion, int nuevoIdRestriccion) {
        try (Connection conn = sql2o.open()) {
            String sql = "UPDATE juegos_restricciones SET id_restriccion = :nuevoIdRestriccion " +
                    "WHERE id_videojuego = :id_videojuego AND id_restriccion = :id_restriccion";
            conn.createQuery(sql)
                    .addParameter("id_videojuego", id_videojuego)
                    .addParameter("id_restriccion", id_restriccion)
                    .addParameter("nuevoIdRestriccion", nuevoIdRestriccion)
                    .executeUpdate();
        } catch (Exception e) {
            System.out.println("Error al actualizar restricción en videojuego: " + e.getMessage());
        }
    }
}
